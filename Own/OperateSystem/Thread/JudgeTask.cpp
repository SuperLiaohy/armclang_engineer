...
void JudgeTask() {
    ...
    auto int_chassis = []() -> int32_t {
        if (interact.chassis.mode == interact_dep::chassis_mode::NONE) {
            return 0;
        }
        return 1;
    };
    auto int_auto = []() -> int32_t {
        if (OneStepGetControl::AUTO == one_step_get_control) {
            return 1;
        }
        return 0;
    };
    ui.operate_fig("001",UI::operation::ADD, UI::layer::LAYER_4, UI::color::GREEN, 2, 24,838,ui_dep::string_data{.font_size = 20,.data = "air:",.length = 5});
    ui.update();
    osDelay(34);
    ui.operate_fig("002",UI::operation::ADD, UI::layer::LAYER_4, UI::color::WHITE, 2, 24,759,ui_dep::string_data{.font_size = 20,.data = "pressure:",.length = 10});
    ui.update();
    osDelay(34);
    ui.operate_fig("003",UI::operation::ADD, UI::layer::LAYER_4, UI::color::CYAN, 2, 24,686,ui_dep::string_data{.font_size = 20,.data = "chassis:",.length = 9});
    ui.update();
    osDelay(34);
    ui.operate_fig("004",UI::operation::ADD, UI::layer::LAYER_4, UI::color::YELLOW, 2, 24,595,ui_dep::string_data{.font_size = 20,.data = "roboarm:",.length = 9});
    ui.update();
    osDelay(34);
    ui.operate_fig("005",UI::operation::ADD, UI::layer::LAYER_4, UI::color::PINK, 2, 24,515,ui_dep::string_data{.font_size = 20,.data = "auto:",.length = 5});
    ui.update();
    osDelay(34);
    ...
    ui.operate_fig(ui_list[4], ui_dep::layer::LAYER_5, UI::color::PINK, 5, 221, 515,

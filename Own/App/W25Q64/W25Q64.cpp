...
uint32_t W25Q64::read_id() {
    ...
    HAL_OSPI_Command(hospi, &sCommand, HAL_OSPI_TIMEOUT_DEFAULT_VALUE);	// 发送指令

    HAL_OSPI_Receive (hospi, OSPI_ReceiveBuff, HAL_OSPI_TIMEOUT_DEFAULT_VALUE);	// 接收数据

    id = (OSPI_ReceiveBuff[0] << 16) | (OSPI_ReceiveBuff[1] << 8 ) | OSPI_ReceiveBuff[2];	// 将得到的数据组合成ID

    return id; // 返回ID
}

W25Q64::W25Q64Status W25Q64::OSPI_W25Qxx_AutoPollingMemReady(void) {
...
W25Q64::W25Q64Status W25Q64::MemoryMappedMode(void)
{
    ...
    // 开启内存映射模式
    if (HAL_OSPI_MemoryMapped(hospi,  &sMemMappedCfg) != HAL_OK)	// 进行配置
    {
...
W25Q64::W25Q64Status W25Q64::write_enable()
{
    OSPI_RegularCmdTypeDef  sCommand;// OSPI传输配置
    OSPI_AutoPollingTypeDef sConfig;// 轮询比较相关配置参数
...
W25Q64::W25Q64Status W25Q64::erase_sector_4(uint32_t addr) {
    OSPI_RegularCmdTypeDef  sCommand;		// OSPI传输配置
    ...
    // 发送写使能
    if (write_enable() != OSPI_W25Qxx_OK)
    {
...
W25Q64::W25Q64Status W25Q64::erase_sector_32 (uint32_t SectorAddress)
{
    OSPI_RegularCmdTypeDef  sCommand;		// OSPI传输配置
...
*
**************************************************************************************************/
W25Q64::W25Q64Status W25Q64::erase_sector_64(uint32_t addr)
{
    OSPI_RegularCmdTypeDef  sCommand;		// OSPI传输配置
...
*
*************************************************************************************************/
W25Q64::W25Q64Status W25Q64::erase_chip (void)
{
    OSPI_RegularCmdTypeDef  sCommand;		// OSPI传输配置
    OSPI_AutoPollingTypeDef sConfig;       // 轮询比较相关配置参数
...
// 读状态寄存器1的第0位（只读），Busy标志位，当正在擦除/写入数据/写命令时会被置1，空闲或通信结束为0
    ...
    if (HAL_OSPI_AutoPolling(hospi, &sConfig,W25Qxx_ChipErase_TIMEOUT_MAX) != HAL_OK)
    {
        return W25Qxx_ERROR_AUTOPOLLING; // 轮询等待无响应
    }
    return OSPI_W25Qxx_OK; // 擦除成功
...
*
***********************************************************************************************************/
W25Q64::W25Q64Status W25Q64::write_page(uint32_t addr, uint8_t *data, uint32_t size)
{
    OSPI_RegularCmdTypeDef  sCommand;// OSPI传输配置
...
W25Q64::W25Q64Status W25Q64::write_buffer(uint32_t addr, uint8_t *data, uint32_t size)
{
    uint32_t end_addr, current_size, current_addr;
    uint8_t *write_data;  // 要写入的数据

    current_size = W25Qxx_PageSize - (addr % W25Qxx_PageSize); // 计算当前页还剩余的空间
    ...
    current_addr = addr;		// 获取要写入的地址
    end_addr = addr + size;	// 计算结束地址
    write_data = data;			// 获取要写入的数据

    do
    {
        // 按页写入数据
        if(write_page(current_addr, write_data, current_size) != OSPI_W25Qxx_OK)
        {

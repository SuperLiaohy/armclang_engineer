...
#define cheat FALSE  //作弊模式 去掉较小的gyro值
#define USING_ZERO_OFFSET 1
#define correct_Time_define 10000    //上电去0飘 1000次取平均
#define temp_times 300       //探测温度阈值

pid_type_def Temperature_PID = {0};
float Temperature_PID_Para[3] = {280, 0.05, 35};
float H7_Temperature = 0.f;
unsigned int adc_v;
float adcx;
...
void INS_Task(void) //1khz
{
    ...
    if ((count % 10) == 0) {
        ...
        H7_Temperature = adcx * (adc_v - *(unsigned short *) (0x1FF1E820)) + 30;
        //========================================================================================
        ...
        } else if (fabsf(temp - RefTemp) > 0.5f) {
            temp_Ticks = 0;
        }
    }
    count++;

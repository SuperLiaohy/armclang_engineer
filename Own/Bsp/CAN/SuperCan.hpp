...
template<typename T>
concept is_can =
    requires(T t, uint16_t filter_number, uint32_t reg1, uint32_t reg2, typename T::filter_mode mode, const uint16_t id,
             const uint8_t* data, int16_t data1, int16_t data2, int16_t data3, int16_t data4) {
        typename T::tx_head;
        typename T::rx_head;
...
public:
    ...
    void filter_config(uint16_t filter_number, filter_mode filterMode = filter_mode::MASK, uint32_t reg_1 = 0,
                       uint32_t reg_2 = 0);
    ...
    inline FDCAN_RxHeaderTypeDef* read_header() { return &rx_header; };

    inline uint8_t* read() { return rx_data; };

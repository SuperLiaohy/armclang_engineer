...
void Chassis::load_speed() {
    ...
    rotateRatio[LeftFront] =
        (front_info.wheel_base + front_info.wheel_track) / 2.f - info.offset_y * abs(move.w) / max.w + info.offset_x * abs(move.w) / max.w;
    rotateRatio[RightFront] =
        (front_info.wheel_base + front_info.wheel_track) / 2.f - info.offset_y * abs(move.w) / max.w - info.offset_x * abs(move.w) / max.w;
    rotateRatio[LeftRear] =
        (rear_info.wheel_base + rear_info.wheel_track) / 2.f + info.offset_y * abs(move.w) / max.w + info.offset_x * abs(move.w) / max.w;
